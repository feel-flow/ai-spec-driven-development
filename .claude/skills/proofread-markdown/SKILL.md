...
##### 問題1：マーカーとテキストの間のスペース

`**` や `*` マーカーと文字の間にスペースがあると正しくレンダリングされません。
...
##### 問題2：日本語括弧との組み合わせ（Vivliostyle等）

**重要**: 一部のレンダラー（Vivliostyle、一部のPDF生成ツール）では、
`**` マーカーが日本語括弧「」と隣接していると正しくレンダリングされません。

```text
🔴 悪い例（Vivliostyleでレンダリングされない）：
AIは**「一般的にウケるデザイン」**を生成する    ← 括弧が**の内側
その鍵が、**「コンテキスト縮小戦略」**です      ← 括弧が**の内側

✅ 良い例（正しくレンダリング）：
AIは「**一般的にウケるデザイン**」を生成する    ← 括弧が**の外側
その鍵が、「**コンテキスト縮小戦略**」です      ← 括弧が**の外側
```

**原因**: `**「` や `」**` の組み合わせを一部のパーサーが正しく解釈できない

**対処法**: 括弧を太字マーカーの外側に配置する
...
| パターン | 問題 | 例 |
| --- | --- | --- |
| `** text**` | 開始マーカー後にスペース | `** 太字**` |
| `**text **` | 終了マーカー前にスペース | `**太字 **` |
| `**「text」**` | 括弧が太字の内側 | `**「重要」**` |
| `**『text』**` | 二重括弧が太字の内側 | `**『重要』**` |
...
# 日本語括弧との組み合わせ問題を検出（Vivliostyle対策）
\*\*「[^」]+」\*\*   # **「...」** パターン
\*\*『[^』]+』\*\*   # **『...』** パターン
```
