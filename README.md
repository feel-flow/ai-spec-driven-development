...
`quality:local` は日々の実行時間のため `npm ci` をチェーンに含めない。クローン直後、または未変更ファイルが `format:md:check` で落ちるときは、先に `npm ci` と `npm --prefix mcp ci` で lockfile を再現する。prettier はキャレットなしの固定バージョン。通過記録の条件と記録先は [`docs/NO_GITHUB_ACTIONS_MIGRATION_DESIGN.md` §3.3](./docs/NO_GITHUB_ACTIONS_MIGRATION_DESIGN.md#quality-local-detail) を参照。
...
/multi-review --cli codex-cli              # Codex だけ（推奨入口とは重ねない）
```

Codex 単体の互換入口は `bash scripts/codex-review.sh --base origin/develop` です（推奨入口の後に実行しない）。ff-dev-toolkit が Codex または Claude に入っていれば plugin cache から自動解決します。`FF_DEV_TOOLKIT_ROOT` は未設定でよい（export した値が残っていると cache より先に勝つ）。`--fresh` はエラーが leftover で止まったときだけ付けます。
