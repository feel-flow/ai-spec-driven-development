...
`coding` と `performance` は現在 0 件でも抽出対象として残す（コード変更の知見が入る枠）。toolkit 側の `domain` / `estimation` は本リポでは未使用のため一覧に載せない。
...
ff-dev-toolkit と同じ**件数導出方式**を採用する（固定 800 行での `playbook/` 分割は行わない。Issue #487）。

- 行数上限 = `ヘッダ行数 + 件数 × 16`（15 行バジェット + ブロック間の空行 1）
