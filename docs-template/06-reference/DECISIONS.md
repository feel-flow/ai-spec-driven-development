...
- **選定理由**: [なぜこのバージョンを選んだか]
- **LTS/サポート期限**: [サポート終了予定日]
- **アップグレード方針**: [次バージョンへの移行タイミング基準]
...
AIツール（Claude Code、GitHub Copilot 等）の知識カットオフによる古い情報の混入を防ぐための対策を記載する。

- **公式ドキュメントURL**: [必ず最新版を参照すべきURL]
- **非推奨API/機能**: [AIが提案しがちだが使用禁止のAPI・パターン]
- **破壊的変更の注意点**: [バージョンアップで変わった重要な仕様]
- **正しい実装例の参照先**: [プロジェクト内の実装例やスニペットのパス]

### AIへの指示

この ADR に基づいてコード生成する際、AIツールは以下を遵守すること。

- **必須**: [この決定に基づいて必ず守るべきルール]
- **禁止**: [この決定に反するため絶対にやってはいけないこと]
- **推奨パターン**: [コード生成時に優先すべき実装パターン]
- **参照すべきファイル**: [実装前に読むべきプロジェクト内ファイル]
