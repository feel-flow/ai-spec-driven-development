...
### 決定
PostgreSQL 16.xを採用する。

### バージョン選定理由
- **なぜ16.xか**: パフォーマンス改善、論理レプリケーション強化
- **AIカットオフ対策**: 2023年リリースのため、主要AIは学習済み
- **非推奨API回避**: 特になし
- **LTS/サポート期間**: 2028年11月まで

### 代替技術との比較
| 技術 | 採用/却下 | 理由 |
|------|----------|------|
| MySQL 8.0 | 却下 | JSON機能がPostgreSQLより弱い |
| MongoDB | 却下 | トランザクション要件に不適合 |
...
### 関連
- 公式ドキュメント: https://www.postgresql.org/docs/16/
```
