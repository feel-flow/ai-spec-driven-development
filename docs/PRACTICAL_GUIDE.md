...
この制約を克服するため、**1つのルーターエージェントに全レビュースキルのロジックを内包する**パターンを採用しました。
...
│  └──────────────┘ └──────────┘ └──────────────┘ │
│  ┌──────────────┐ ┌──────────┐ ┌──────────────┐ │
│  │ Type Design   │ │ Comment  │ │ Code         │ │
│  │ Analysis     │ │ Analysis │ │ Simplify     │ │
│  │ (条件付き)    │ │ (条件付き)│ │ (条件付き)    │ │
│  └──────────────┘ └──────────┘ └──────────────┘ │
│                          │                       │
│                          ▼                       │
...
1. **単一エージェント内で完結**: `.agent.md` ファイル1つの中にすべてのレビューロジックを定義
...
| 観点 | .agent.md（宣言的） | Extension API（プログラマティック） |
| ------ | --------------------- | ------------------------------------- |
| 導入コスト | 低（Markdownのみ） | 高（TypeScript開発必要） |
| 柔軟性 | 中（LLM依存） | 高（完全制御） |
| メンテナンス | 容易 | 開発・テスト必要 |
| スキル間連携 | 不可（ルーターで解決） | 可能（invokeTool） |
