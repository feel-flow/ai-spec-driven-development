...
バックグラウンドでは出力を `tail` / `grep` / `head` へパイプしない。パイプするとプロセス終了までバッファされ、進捗確認のために出力ファイルを読んでも空のままになる。途中失敗に気づけない。「パイプしない」と知っていても、長い出力を要約したい動機のほうが強く再発する。出力は素のまま流し、絞り込みは完了後に出力ファイルへ対して行う。途中経過を絞って見たい場合は、バックグラウンドにせず timeout 付きで前景実行する（観測: [OBS-023](./08-knowledge/OBSERVATIONS.md#obs-023)）。[OBS-016](./08-knowledge/OBSERVATIONS.md#obs-016) の終了コード取りこぼしとは機構が別である。

```bash
# ❌ 終了までバッファされ、途中の出力ファイルが空になる
npm run quality:local 2>&1 | tail -25

# ✅ 出力は素のまま流す。絞り込みは完了後に出力ファイルへ
npm run quality:local
# 完了後: grep -E "error|FAIL" <出力ファイル> | tail -8
```
