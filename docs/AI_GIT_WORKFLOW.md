...
バックグラウンドでは出力を `tail` へパイプしない。`tail -n` は末尾を確定するために EOF を待つので、進捗確認のために出力ファイルを読んでも終了まで空のままになる。途中失敗に気づけない。「パイプしない」と知っていても、長い出力を要約したい動機のほうが強く再発する。出力は素のまま流し、絞り込みは完了後にホストが保存した出力ファイルへ対して行う。途中経過を絞って見たい場合は、バックグラウンドにせず timeout 付きで前景実行する（観測: [OBS-023](./08-knowledge/OBSERVATIONS.md#obs-023)）。`grep` は非 TTY でブロックバッファし、`head` は指定行数後にパイプを閉じて上流を SIGPIPE しうる。どちらも `tail -n` の EOF 待ちとは別の挙動なので、同じ「終了までバッファ」とは扱わない。いずれにせよバックグラウンドのコマンド自体をパイプで加工しない。[OBS-016](./08-knowledge/OBSERVATIONS.md#obs-016) の終了コード取りこぼしとは機構が別である。

```bash
# ❌ tail -n は EOF 待ち。バックグラウンドの出力ファイルは終了まで空
npm run quality:local 2>&1 | tail -25

# ✅ バックグラウンドではコマンドを素のまま起動する（ホストが出力ファイルへ保存）
npm run quality:local

# 完了後、ホストが保存した出力ファイルを絞る
# （$OUTPUT_FILE は説明用。Claude Code の Bash output / Grok の terminal log 等）
grep -E "error|FAIL" "$OUTPUT_FILE" | tail -8
```
