...
長時間コマンドを `cmd | tail -6` のようにパイプしてバックグラウンド起動すると、出力がプロセス終了までバッファされ、進捗確認のために出力ファイルを読んでも空のままになる。途中で失敗していても早期に気づけない → バックグラウンドでは出力を加工せず素のまま流し、絞り込みは完了後に出力ファイルへ対して行う。「パイプしない」という知識だけでは、長い出力を要約したい動機のほうが強く再発する。[OBS-016](#obs-016) の zsh `PIPESTATUS` 由来とは機構が別で、対処も別（あちらは終了コードの取り方、こちらは途中経過の可視性）。
