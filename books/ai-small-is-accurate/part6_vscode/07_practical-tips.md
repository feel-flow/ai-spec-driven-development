...
user/
├── user-type.ts        (型定義)
├── user-constants.ts   (定数)
├── user-validator.ts   (バリデーション)
├── user-mapper.ts      (マッピング)
├── user-repository.ts  (DB操作)
├── user-service.ts     (ビジネスロジック)
├── user-controller.ts  (API)
└── user-helper.ts      (ヘルパー)

→ 8ファイルに分散、1つの変更で複数ファイルを渡す必要
```
...
### 図7-1：ファイルサイズとAI精度の関係

```
スタイル: クリーンなインフォグラフィック
内容:
- X軸: ファイルサイズ（100行、500行、1000行、2000行）
- Y軸: AI精度（%）
- 曲線が500行あたりから急激に下降
- 500行のところに「最適ゾーン」のマーキング
配色: 青系グラデーション
```

### 図7-2：ファイル分割のBefore/After
