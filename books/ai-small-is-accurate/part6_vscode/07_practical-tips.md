...
- ファイル分割がAI精度に与える影響
- 効果的なファイル設計の考え方
- VSCode + AI（Claude Code）の実践的な使い方
- この本の執筆プロセス（メタな紹介）
...
AIツールにとって、「見える範囲」とはファイルです。1つの大きなファイルに全てを詰め込めば、AIは大量のコンテキストを処理しなければなりません。第2章で学んだ「Lost in the Middle」問題が発生し、中間部分を忘れてしまいます。

逆に、適切なサイズにファイルを分割すれば、AIは必要な情報だけを見て、正確に作業できます。

![大きなファイル vs 分割されたファイル。大きな2000行のファイルはAIが中間を忘れて精度が下がるが、200-300行に分割されたファイルならAIは必要なファイルだけを見て精度が上がる。](./07_file_splitting_effect.png)
...
![1ファイル複数責務 vs 責務ごとに分割。悪い例では1000行を超えてAIが全体を把握しにくいが、良い例では責務ごとに分割して各ファイル200-300行でAIが正確に把握できる。](./07_responsibility_split.png)
...
![過度な分割 vs 適度なグループ化。悪い例では8ファイルに分散して1つの変更で複数ファイルを渡す必要があるが、良い例では4ファイルに集約して関連情報が近くにある。](./07_grouping_balance.png)
...
![コンテキスト配分の優先順位と目安。優先順位は高い順に、修正対象のファイル、直接依存するファイル、型定義ファイル、関連するテストファイル、設定ファイル、READMEや仕様書。配分の目安は修正対象50%、直接依存30%、型定義10%、その他10%。](./07_context_priority.png)

---

## 7.5　VSCode + Claude Codeの実践
...
- 「第3部と第4部の対比がわかりにくい」
- 「Lost in the Middleは重要なので独立した章に」
- 「実践編が欲しい」
...
core_concepts:
  ...
  - standard: "Lost in the Middle"
    variants:
      ...
      - "ロスト・イン・ザ・ミドル"
    description: "LLMが長い入力の中間部分を忘れやすい問題"
```
...
![AI侍とDJ町娘がファイル分割について対話するイラスト](../images/characters.png)

**DJ町娘**：「師匠、なんで500行なんですか？400行じゃダメなの？」

**AI侍**：「良い質問である。500行というのは、厳密な数字ではない。**人間が一目で把握できる量**という意味なのだ」

**DJ町娘**：「人間が？」

**AI侍**：「うむ。AIに渡す前に、人間がファイルを開いて**全体像を把握できる**かどうか。スクロールが何度も必要なら、それはAIにとっても難しいということだ」

**DJ町娘**：「なるほど...。でも分けすぎるのも良くないんですよね？」

**AI侍**：「その通り。**一緒に変わるものは、一緒に置く**。これが分割の極意だ。ユーザーの登録処理と退会処理は一緒に変わることは少ない。だから分ける。しかし、型定義とバリデーションは密接に関連している。だから一緒にする」

**DJ町娘**：「わかりました！分割は**変更の単位**で考えればいいんですね✨」
