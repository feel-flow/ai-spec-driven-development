...
## ◼︎「ChatGPTに5000字書いてもらったら、なんか微妙だった」話
...
![タスクサイズと視認性の比較](./images/flashlight_comparison.png)

AIに大きなタスクを渡すと、こうなります:

![コンテキストサイズと精度の関係](./images/context_accuracy_chart.png)
...
実は、AIには**もう1つの弱点**があります。2025年に発見された「Lost at the Beginning of Reasoning（推論の最初で迷子になる）」問題です。これは「熟考型AI」（推論モデル）に特有の弱点で、2-2で詳しく解説します。
...
### ケーススタディまとめ：3つの失敗に共通するもの

3つのケースを振り返ると、すべてに共通するパターンが見えてきます。

| ケース | 依頼内容 | 結果 | 問題の本質 |
|--------|---------|------|-----------|
| 市場調査 | 「包括的なレポートを作って」 | 表面的な情報の羅列 | **深さが足りない** |
| コード実装 | 「機能を全部実装して」 | プロジェクト標準を無視 | **文脈が伝わっていない** |
| プレゼン資料 | 「10-15枚のスライドで」 | 汎用的で刺さらない | **具体性がない** |

**共通点**: どれも「大きく、一度に」頼んでいる。

そして、どれも「70%の完成度」で、残り30%の修正に膨大な時間がかかっている。

**教訓**:
AIは「大きな依頼」を受けると、**広く浅く**対応しようとします。
結果として、深さ・文脈・具体性が失われるのです。

<div class="column-box">

### 🛒 コラム：スーパーの買い物袋は1つで大丈夫？

![スーパーの買い物袋は1つで大丈夫？](./images/column_01-1_shopping_bag.png)

**教訓**: 大きな依頼1つ（70%成功）より、小さな依頼3つ（100%×3）の方が確実。買い物もAIも、欲張りは禁物！🛍️
