...
![スコープ収束パターンのフローチャート：大きなゴール（100%）を4つのマイルストーンに分解し、各マイルストーンを完了後、98%精度の完成品に収束する流れを示す図](./03_scope_convergence_pattern.png)
...
**AI侍**: 「ふむ、スコープ収束パターンの極意を授けよう」

**DJ町娘**: 「AIに『全部やって』って頼むと70%で止まるんですよね。どうすればいいんですか？」

**AI侍**: 「簡単である。『小さく分けて渡す』のだ」
...
```text
❌ 「ログインとログアウトとパスワードリセットを実装して」
✅ 「まずログイン機能を実装して」→確認→「次にログアウトを」
```
...
```text
✅ 「ログイン機能を実装して」→ 実際にログインして確認できる
❌ 「セキュリティを向上させて」→ 何をどう確認する？
```

**DJ町娘**: 「なるほど！『1つの成果物』『検証可能』がポイントですね✨」

**AI侍**: 「うむ。分けすぎも禁物だ。関数名だけ決めて、引数を1つずつ追加して...では文脈が断絶する」

**DJ町娘**: 「ちょうどいい粒度が大事なんですね！」

**AI侍**: 「その通り。次章では『検証可能にする』をさらに深掘りするぞ。AIにツールを作らせて、それで検証する──二段階AI活用の極意だ」
