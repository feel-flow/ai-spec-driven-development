...
Decision Matrixを使いこなす3つの極意を授ける。
...
1. **誰のための情報か？**
   - ビジネス（ステークホルダー）→ PROJECT / DOMAIN
   - 技術（開発者）→ ARCHITECTURE / PATTERNS / TESTING / DEPLOYMENT
   - 全員（ナビゲーション）→ MASTER

2. **何を定義しているか？**
   - Why/What → PROJECT / DOMAIN
   - How（技術）→ ARCHITECTURE
   - How（実装）→ PATTERNS
   - 正しさ → TESTING
   - 運用 → DEPLOYMENT
...
#### 秘伝その2：迷ったら「読み手」で判断せよ

それでも迷ったら、**誰が一番それを読むか**で判断せよ。

- **AIが頻繁に参照する** → PATTERNS / ARCHITECTURE
- **レビュアーが確認する** → DOMAIN / TESTING
- **新メンバーが最初に読む** → MASTER / PROJECT

読み手の視点で考えれば、自然と配置先が見える。

#### 秘伝その3：関連文書にもメモを残せ

情報を1箇所に置いたら、**関連する文書にも「参照先」を書け**。
...
関連：
- TESTING.md: レート制限のテストケース
- DEPLOYMENT.md: レート制限のモニタリング設定
- DOMAIN.md: レート制限エラーのメッセージ定義
```

これで「ここに書いてないなら、どこを見ればいい？」が分かる。
