...
すべての変更に同じレベルの確認作業を行うのは非効率です。文言修正に全チームレビューは過剰ですし、認証方式の変更を1人で判断するのは危険です。

変更の影響度を3段階に分類することで、**確認作業のコストと変更のリスクを適切にバランス**させます。分類があれば、「この変更にはどこまで確認が必要か」が即座に判断でき、チーム全体で共通の基準を持てます。
...
既存の機能に波及する可能性があり、関連する文書やコードの確認が必要な変更です。
...
設計の根本に関わる変更で、複数チーム・複数文書への波及と移行計画が必須です。
...
AI仕様駆動開発の大きな利点は、**仕様書を更新すればAIが実装を再生成できる**ことです。しかし、この「再生成」を無計画に繰り返すと、意図しない箇所が壊れたり、変更の影響が追えなくなります。

ここでは、影響度評価と組み合わせて、仕様変更からAI再実装までを**安全に繰り返せる仕組み**を解説します。
...
HIGH変更は一気にデプロイしません。段階的にリリースし、各段階で問題が検出されたら即座にロールバックできる状態を維持します。

![段階的なデプロイ](../images/staged-deploy.png)

### AIへの指示テンプレート

3原則を踏まえたうえで、AIに再実装を依頼する際は**影響度と制約を明示**することが重要です。曖昧な指示では、AIが影響範囲外のコードまで変更してしまうリスクがあります。以下のテンプレートを使うことで、変更のスコープを明確に伝えられます。
...
変更を恐れず、安全に進める3つの極意を授ける。

#### 秘伝その1：変更前に影響度を判定せよ

コードを変更する前に、まず**影響度**を判定せよ。
...
影響度が分かれば、どこまで確認すべきかが分かる。

#### 秘伝その2：HIGHには必ずADRを書け

HIGH影響度の変更には、**必ずADRを書け**。
...
ADRがあれば、将来「なぜこうなったの？」が分かる。

#### 秘伝その3：「ドキュメント先、コード後」の習慣を持て

変更の流れを逆転せよ。

**従来**：コード変更 → （忘れた頃に）ドキュメント更新
**新しい**：**ドキュメント変更 → コード変更**

仕様を先に変更すれば、AIは**最新の仕様に従った実装**をする。
後からドキュメントを直すと、AIは**古い仕様のまま実装**してしまう。
