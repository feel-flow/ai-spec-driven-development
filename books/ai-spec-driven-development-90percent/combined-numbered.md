...
本章と次章（第12a章）では、**Claude Code**と**GitHub Copilot**の両方を取り上げます。

どちらのツールでも、仕様駆動開発を支援する**同等のエージェント機能**を利用できます。普段お使いのツールに合わせて、該当する章を参照してください。

| 章 | 対象ツール | 内容 |
|----|-----------|------|
| 本章（第12章） | Claude Code | Skills、pr-review-toolkit |
| 次章（第12a章） | GitHub Copilot | カスタムエージェント |
...
## 12.53　pr-review-toolkit（公式プラグイン）
...
### 12.54.1　1. 起動条件（description）の重要性
...
### 12.59.1　うまくいかなかったスキルの事例

スキル設計で実際に起きた失敗例を紹介します。同じ轍を踏まないための参考にしてください。

#### 12.59.1.1　descriptionが曖昧で誤発動
...
## 12.60　章末チェックリスト（Claude Code ユーザー向け）
...
この章では、Claude Code Skillsを使って仕様駆動開発を自動化する方法を学びました。

次章（第12a章）では、**GitHub Copilot Agents**を取り上げます。Claude Codeを使わない方も、同等のレビュー機能を実現できることを確認しましょう。
...
## 12a.14　Code Review Results

### 12a.14.1　Critical Issues (信頼度 91-100)
- [ファイル名:行番号] 問題の説明
  - 信頼度: XX
  - 理由: なぜこれが問題か
  - 修正提案: どう修正すべきか

### 12a.14.2　Important Issues (信頼度 80-90)
- [ファイル名:行番号] 問題の説明
  - 信頼度: XX
  - 理由: なぜこれが問題か
  - 修正提案: どう修正すべきか
...
### 12a.15.1　2. error-handler-hunter.agent.md
...
## 12a.20　Error Handling Analysis Results

### 12a.20.1　CRITICAL Issues
- [ファイル名:行番号] 問題の説明
  - コード: 問題のあるコード
  - 問題: 何が問題か
  - 修正提案: 推奨される修正
...
### 12a.20.3　3. test-analyzer.agent.md
...
## 12a.25　Test Coverage Analysis Results

### 12a.25.1　Critical Gaps (優先度 9-10)
...
### 12a.25.2　4. code-simplifier.agent.md
...
## 12a.29　Code Simplification Results

### 12a.29.1　Simplification Opportunities
- [ファイル名:行番号]
  - 現在のコード: ...
  - 提案: ...
...
### 12a.30.1　5. comment-analyzer.agent.md
...
## 12a.34　Comment Analysis Results

### 12a.34.1　Critical Issues（事実として不正確）
- [ファイル名:行番号]
  - コメント: "..."
  - 問題: コメントが実際のコードと矛盾
  - 推奨修正: ...

### 12a.34.2　Improvement Opportunities（改善可能）
- [ファイル名:行番号]
  - 問題: 情報が不完全
  - 推奨追加内容: ...
```

### 12a.34.3　6. type-design-analyzer.agent.md
...
## 12a.39　Type Design Analysis Results
...
## 12a.43　Claude Code vs GitHub Copilot 比較
...
## 12a.44　章末チェックリスト（GitHub Copilot ユーザー向け）
