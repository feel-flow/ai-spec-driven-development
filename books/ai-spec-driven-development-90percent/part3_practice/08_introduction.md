...
### ステップ4：残りの文書は「AIと対話しながら」埋める

最初から完璧な文書を作る必要はありません。まずは**見出しだけ**で構造を確保しましょう。
...
ただし、**見出しだけで放置してはいけません**。空欄のままにしておくと、AIは推論で勝手に埋めてしまいます。これは第1部で見た「vibe coding」と同じ構造の問題です。

大事なのは「場所を確保した上で、AIと対話しながら埋めていく」ことです。自分一人で完璧に書こうとする必要はありません。AIに「ARCHITECTURE.mdのシステム構成を一緒に考えよう」と相談すれば、選択肢を提示してくれます。その中から選び、判断の理由を文書に残す——この対話プロセス自体が仕様策定です。

ここで意識してほしいのが、**「推論されてOKな部分」と「推論されたら困る部分」の区別**です。

| 推論に任せてOK | 事前に明記すべき |
| -------------- | ---------------- |
| テストのディレクトリ構造 | ビジネス上の制約条件 |
| コードフォーマットの設定 | 認証・認可の要件 |
| ログ出力のフォーマット | データの保持期間・削除ポリシー |
| CIパイプラインの基本構成 | 外部サービスとの連携仕様 |

推論されてOKな部分は、AIが一般的なベストプラクティスに従って埋めてくれます。しかし、**ビジネス固有の判断が必要な部分**をAIに推論させると、もっともらしいが間違った仕様が生まれます。「それっぽいけど違う」は、「明らかに間違っている」よりも発見が遅れるぶん厄介です。

**迷ったら、AIに聞いてみましょう。**「この項目は推論で埋めて大丈夫？」と質問すれば、AIは「この部分はビジネス要件に依存するので、確認したほうがよいです」と教えてくれます。AIを"書き手"としてだけでなく、"壁打ち相手"として活用するのが実践のコツです。
