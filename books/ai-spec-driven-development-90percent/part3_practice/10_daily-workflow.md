...
![AI開発フロー](../images/ch10-daily-workflow.jpeg)
...
| 形式 | 拡張子 | 特徴 |
| ---- | ------ | ---- |
| Markdown形式 | `.md` | 自由記述。AIが読みやすく、柔軟にカスタマイズできる |
| YAML形式（Issue Forms） | `.yml` | フォーム形式。入力欄の必須化やドロップダウンが使える |

YAML形式はGitHub.comのWeb UIでのみ有効で、APIやCLI経由でのIssue作成には適用されません。本書ではAIとの親和性が高く、どの環境でも使えるMarkdown形式を推奨します。

各Markdownファイルの先頭にYAML frontmatterを記述すると、テンプレート名・ラベル・タイトルプレフィックスが自動設定されます。
...
#### 設計判断が必要なとき——ADRを書くタイミング

確認2の段階で、**既存の設計制約では判断できない新しい問題**に遭遇することがあります。たとえば：

- 既存の認証方式では対応できない要件が出てきた
- 新しい外部APIとの連携方式を決める必要がある
- パフォーマンス要件のために、キャッシュ戦略を変更したい

こうした場面が、**ADR（Architecture Decision Record）を書くタイミング**です。

```markdown
## ADR-007: 通知配信にWebSocketを採用

### ステータス
提案中

### コンテキスト
Issue #58「リアルタイム通知機能」の実装にあたり、
配信方式を決定する必要がある。

### 検討した選択肢
1. ポーリング（5秒間隔）：実装が簡単だが、サーバー負荷が高い
2. SSE（Server-Sent Events）：一方向のみ、導入は容易
3. WebSocket：双方向、既存のインフラとの相性が良い

### 決定
WebSocketを採用する。理由：既にSocket.ioがインフラに
導入済みであり、将来的なチャット機能との統合も見据える。

### AIへの指示
通知配信にはWebSocket（Socket.io）を使用すること。
ポーリングやSSEは使用しない。
```

ADRは**Issueに着手する前に書く**のがベストです。設計判断が決まっていない状態でAIにタスクを渡すと、AIが推測で実装してしまい、後から「実はこの方式じゃなかった」と手戻りが発生します。

ADRの詳しい書き方は第4章で、変更時のADR運用は第7章で解説しています。
...
「Issue #42を実装して」と伝えただけで、AIは以下を自動的に実行します。

1. Issueを読んで受け入れ基準を理解する
2. ブランチを作成する
3. 7文書を参照しながらコードを書く
4. コミットしてPRを作成する

「そんなこと、一言で伝わるわけがない」と思うかもしれません。実は、これを可能にしているのが**AIエージェント設定ファイル**です。

AIコーディングツールは、起動時にリポジトリルートの設定ファイルを**自動的に読み込みます**。Claude Codeなら`CLAUDE.md`、GitHub CopilotやCursorなら`AGENTS.md`です。この設定ファイルに**git workflowが事前定義**されているため、AIは「Issueを受けたら何をすべきか」を最初から知っています。

つまり、AIが受け取るコンテキストは3層構造になっています。

```text
┌─────────────────────────────────────────────────┐
│  Layer 1: 設定ファイル（常に自動読み込み）         │
│  CLAUDE.md / AGENTS.md                           │
│  → Git workflow、コーディングルール、必読文書      │
├─────────────────────────────────────────────────┤
│  Layer 2: 仕様文書（設定ファイルから参照）         │
│  7文書（MASTER.md → PATTERNS.md → ...）          │
│  → アーキテクチャ、ドメイン、実装パターン         │
├─────────────────────────────────────────────────┤
│  Layer 3: タスク指示（あなたの一言）              │
│  「Issue #42 を実装して」                         │
│  → 受け入れ基準、技術的制約、スコープ外           │
└─────────────────────────────────────────────────┘
```

Layer 1の設定ファイルとLayer 2の仕様文書が**事前に整備されている**から、あなたが渡すLayer 3はたった一言で済むのです。逆に言えば、設定ファイルがなければ、AIは毎回「ブランチ名はどうしますか？」「コミットメッセージの規約は？」と聞き返すか、推論で勝手に決めてしまいます。

設定ファイルのgit workflow定義は、たとえばこのようなものです。
...
この数行があるだけで、AIの振る舞いは劇的に変わります。「Issue #42を実装して」と言えば、AIは自動的に`feature/#42-login-api`のようなブランチを作り、Conventional Commits形式でコミットし、developブランチ宛てのPRを作成します。

> **補足**: 設定ファイルの作成方法は第8章のステップ1.5で解説しました。まだ作成していない方は、先に設定ファイルを用意してから本ステップに戻ってください。

### ツールが自動読み込みする設定ファイル

主要なAIコーディングツールと、それぞれが自動読み込みする設定ファイルの対応は以下のとおりです。
...
ほとんどのツールが**AGENTS.md**（Linux Foundation傘下のオープンスタンダード）に対応しています。複数のツールを併用するチームでは、`CLAUDE.md`と`AGENTS.md`の両方を用意するとよいでしょう。各ツールの詳しい設定方法は「付録：AIエージェント設定ファイル一覧」を参照してください。
...
設定ファイルと7文書が整備されていれば、あなたが実行するコマンドはこれだけです。
...
Issueをテンプレート化したのと同じ発想で、PRにもテンプレートを用意します。GitHubでは `.github/PULL_REQUEST_TEMPLATE.md` に配置するだけで、PR作成時に自動で適用されます。
...
ポイントは「**確認ポイント**」セクションです。レビュワー — 人間でもAIでも — が何を見ればいいかを、PR作成者が明示します。

Issueテンプレートが「何を作るか」の型なら、PRテンプレートは「何を確認するか」の型です。この2つが揃うと、開発サイクル全体に型が通ります。
...
PRを出したら、レビューエージェントにレビューを依頼します。

AIコーディングツールの「スキル」や「プラグイン」機能を使うと、スコープを絞った専門レビュワーを作れます。たとえば：

- **セキュリティレビュワー** — 認証・認可・入力検証に特化
- **パフォーマンスレビュワー** — N+1クエリ・メモリ効率に特化
- **アーキテクチャレビュワー** — レイヤー構成・責務分離に特化

使い方はIssueのときと同じで、短い指示だけです。

```text
このPRをレビューして
```

これだけで、各専門レビュワーがPRの差分を読み、ファイル名・行番号つきで指摘を返してきます。人間のレビュワーに「ここ見てください」とお願いするのと同じ感覚で、AIレビュワーにも依頼できるわけです。

レビュー手順をスキルとして定義しておけば、`/review-pr` のようにスキル名を直接指定して呼び出すこともできます。各レビュワーの詳しい実装方法は **第11章** で解説します。

### 指摘を受けたら修正する

レビューエージェントの指摘は具体的です。ファイル名、行番号、理由がついてきます。だから、AIにそのまま渡せます。

```text
レビュー指摘に対応して
```

ステップ3で「Issue #42を実装して」と短い指示で済んだのと同じです。レビュー指摘には**具体的な問題と期待される解決方向**がすでに含まれているため、AIは的確に修正できます。

### このループは自動化できる

PR作成 → レビュー → 修正 → 再レビュー。このサイクルはGitHub Actions等のCI/CDで自動化することもできます。

たとえば、PRが作成されたらレビューエージェントが自動起動し、指摘があればコメント、なければApprove — という流れです。

自動化の詳細は **第11章** と **第15章** で解説します。
...
![AI侍道場：Issueは芸術である](../images/ch10-ai-samurai-dojo.png)
