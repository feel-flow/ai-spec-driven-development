...
Issueをテンプレート化したのと同じ発想で、PRにもテンプレートを用意します。GitHubでは `.github/PULL_REQUEST_TEMPLATE.md` にテンプレートを配置するだけで、PR作成時に自動的にテンプレートが適用されます。
...
ポイントは **「確認ポイント」セクション** です。レビュワー — 人間でもAIでも — が何を見ればいいかを、PR作成者が明示します。
...
AIコーディングツールの「Skills」や「プラグイン」機能を使うと、スコープを絞った専門レビュワーを作れます。たとえば：
...
レビュー手順をスキルとして定義しておけば、`/review-pr` のようにスキル名を直接指定して呼び出すこともできます。

各レビュワーの詳しい実装方法は **第12章** で解説します。
...
PR作成 → レビュー → 修正 → 再レビュー。このサイクルはGitHub Actions等のCI/CDで自動化することもできます。たとえば、PRが作成されたらレビューエージェントが自動起動し、指摘があればコメント、なければApprove — という流れです。

自動化の詳細は **第12章** と **第13章** で解説します。
