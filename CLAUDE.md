...
| 項目 | 値 | 備考 |
|------|-----|------|
| トリムサイズ | 5.83" × 8.27"（A5） | `build-paperback-cover.js` の `trimWidth` / `trimHeight` |
| ブリード | 0.125"（各辺） | 印刷後にカットされる領域 |
| セーフゾーン | 0.374"（9.5mm） | トリムエッジからテキストまでの最小距離 |
| ファイル端からの最小距離 | 0.499"（≈150px） | bleed + safeZone |
| 背幅 | ページ数 × 0.0025"（クリーム紙） | `pageCount * pageThickness` |

### バーコードエリア仕様

- **サイズ**: 50.8mm × 30.5mm（2" × 1.2"）
- **色**: 明るい単色（白）— テキストや画像を被せない
- **配置**: 裏表紙の**左下**（見開きで裏表紙は左側、外端＝左端）
- **マージン**: セーフゾーン分（0.499"）を端から確保
...
# 例: node build-paperback-cover.js 368
# → images/cover_paperback.jpg / cover_paperback.pdf を生成
```
...
| 指摘内容 | 原因 | 対処 |
|---------|------|------|
| テキストが端に近すぎる | セーフゾーン未確保 | 全テキストをbleed+safeZone（0.499"≈150px）以上内側に配置 |
| バーコードスペースなし | バーコード領域が不足・テキスト被り | 裏表紙左下に2"×1.2"の白い矩形を確保 |
| 背幅が合わない | ページ数変更 | 最終ページ数でスクリプトを再実行 |